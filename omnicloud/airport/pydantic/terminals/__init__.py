'''Terminals for pydantic objects.'''
...
from omnicloud.airport.abc import Building as _Building
from omnicloud.airport.tools.type import underscore_to_camelcase as _2camelcase
...
def _import_terminals(start_dir, package_name):
    with _scandir(start_dir) as entries:
        for entry in entries:
            if (
                entry.is_dir()
                and entry.name.startswith('_')  # noqa: W503
                and _path.isfile(_path.join(entry.path, '__init__.py'))  # noqa: W503
            ):
                ...
                # Add the subpackage to the omnicloud.airport.pydantic.terminals namespace
                setattr(_modules[__name__], entry.name, subpackage)

                subpackage.__package__ = subpackage_name
                spec.loader.exec_module(subpackage)

                terminal = getattr(subpackage, 'Terminal')  # noqa: B009
                ...
                infobox[terminal_name]['description'] = (
                    str(terminal.__doc__).strip().split('\n', 1)[0][:125].strip()
                )
...
if _env.get('OMNICLOUD_AIRPORT_DEVPATH', '') != '' and _path.isdir(_env['OMNICLOUD_AIRPORT_DEVPATH']):
    _workdir = _path.join(_env['OMNICLOUD_AIRPORT_DEVPATH'], 'omnicloud/airport/pydantic/terminals')
